...
export class ChatService {
  ...
  private buildPersonalizedIntro(userMessage: string): string {
    if (!this.memoryService) return '';
    try {
      const recall = this.memoryService.buildRecallContext(userMessage, 5);
      if (!recall.facts.length) return '';
      ...
      const parts: string[] = [];
      if (normalized.name) parts.push(`Aapka naam ${normalized.name} hai`);
      if (normalized.role) parts.push(`Aap ${normalized.role} ho`);
      if (normalized.goal) parts.push(`aapka goal ${normalized.goal} hai`);
      if (normalized.preference) parts.push(`aapko ${normalized.preference} pasand hai`);
      if (normalized.location) parts.push(`aap ${normalized.location} se ho`);
      return parts.length ? `Astro note: ${parts.slice(0, 3).join(', ')}.` : '';
    } catch {
      return '';
    }
  }

  private applyPersonalization(userMessage: string, response: string, pathOverride?: ChatPath): string {
    const intro = this.buildPersonalizedIntro(userMessage);
    const base = intro ? `${intro}\n\n${response}` : response;
    if (this.shouldKeepDetailedTone(userMessage, pathOverride)) {
      return base;
    }
    return this.formatFriendResponse(base);
  }

  private shouldKeepDetailedTone(message: string, path?: ChatPath): boolean {
    if (path === 'deep') return true;
    if (this.isCodeAwarenessRequest(message)) return true;
    if (this.isArchitectureRequest(message)) return true;
    if (this.isSelfUpgradeRequest(message)) return true;
    return false;
  }

  private isFriendMode(message: string): boolean {
    if (!message) return false;
    return /\b(friend|friend\s?mode|dost|yaar|buddy|gf|pal|dosto)\b/i.test(message);
  }

  private formatFriendResponse(text: string): string {
    const sentences = text.split(/(?<=[.!?])\s+/).filter((s) => s.trim());
    const short = sentences.slice(0, 2).join(' ').trim();
    if (!short) return 'Boss, main yahan hoon, jo bhi chahiye bolo.';
    if (short.length > 160) {
      return `${short.slice(0, 160).replace(/\s+$/u, '')}...`;
    }
    return short;
  }
  ...
  async processMessageWithMeta(sessionId: string, userMessage: string): Promise<{ response: string; sources: string[]; confidence: number }> {
    ...
    const finalResponse = this.applyPersonalization(userMessage, out.response, path);
    this.addMessage(sessionId, 'assistant', finalResponse);
    this.storeLearning(sessionId, userMessage, finalResponse, undefined, this.detectTags(userMessage));
    return { response: finalResponse, sources: out.meta.sources, confidence: out.meta.confidence };
  }
