...
export class VectorStoreService {
  ...
  private buildFingerprint(): string {
    const parts: string[] = [];
    const appendDirState = (dirPath: string, ext: string) => {
      if (!fs.existsSync(dirPath)) return;
      const files = fs.readdirSync(dirPath).filter((f) => f.endsWith(ext)).sort();
    ...
    appendDirState(LEARNING_DATA_DIR, '.txt');
    appendDirState(CHATS_DATA_DIR, '.json');
    return parts.join('|');
  }

  private ensureFresh(): void {
    const fp = this.buildFingerprint();
    if (!this.chunks.length || fp !== this.lastFingerprint) {
