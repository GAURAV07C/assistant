...
- **Primary runtime (advanced)**: `ts-backend` + `react-assistant-ui` + `vscode-Astro-extension`
- **Legacy runtime (simple/parallel)**: Python `app/` stack

If your goal is the full personal advanced assistant (agent + evolution + skills + dashboard), use the TypeScript runtime.

## TypeScript Runtime (Recommended)
...
- Run full advanced stack:
```bash
npm run dev
```

- Run only Next.js dashboard UI:
```bash
npm run dev:ui
```
...
### Chat System (TS Backend)

- Core files:
  - `ts-backend/src/server.ts` (chat routes + SSE + adaptation)
  - `ts-backend/src/core/agent/controller.ts` (intent/mode/clarification/style logic)
  - `ts-backend/src/services/chatService.ts` (chat session + stream orchestration)
...
- Auto behavior:
  - backend auto-detects `intent`, `complexity`, `ambiguity`
  - auto-selects `casual` vs `strategic` mode
  - asks clarification only when really needed
  - small-talk (`hi`, `hlo`, `ok`, `thanks`) stays natural and non-formal

### React Assistant UI (Next.js + TSX)
...
- Modular tab components:
  ...
  - `react-assistant-ui/src/components/dashboard/tabs/AgentTab.tsx`
  - `react-assistant-ui/src/components/dashboard/tabs/SkillsTab.tsx`
  - `react-assistant-ui/src/components/dashboard/tabs/FilesTab.tsx`
  - `react-assistant-ui/src/components/dashboard/tabs/VoiceTab.tsx`
  - `react-assistant-ui/src/components/dashboard/tabs/LogsTab.tsx`

- UI capabilities:
  - live chat streaming + realtime toggle
  - agent plan/execute views
  - skills evolution controls
  - learning/workspace file editor
  - voice settings and custom voice upload
  - audit and memory inspection dashboard
