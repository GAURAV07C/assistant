...
let currentAgentMode = 'strategic';
...
const agentExecuteBtn = $('agent-execute-btn'); // Trigger structured /agent/execute flow
...
const btnAgentCasual = $('btn-agent-casual');
const btnAgentStrategic = $('btn-agent-strategic');
...
const agentPanelToggle = $('agent-panel-toggle');
const agentPanelWidget = $('agent-panel-widget');
const agentPanelClose = $('agent-panel-close');
const agentPanelSummary = $('agent-panel-summary');
const agentPanelContract = $('agent-panel-contract');
const agentPanelEval = $('agent-panel-eval');
const agentPanelSteps = $('agent-panel-steps');
const agentPlanBtn = $('agent-plan-btn');
const agentExecBtnPanel = $('agent-exec-btn-panel');
...
function openAgentPanel() {
    if (!agentPanelWidget) return;
    agentPanelWidget.classList.add('open');
}

function closeAgentPanel() {
    if (!agentPanelWidget) return;
    agentPanelWidget.classList.remove('open');
}

function formatJsonCompact(obj, fallback = '-') {
    if (!obj) return fallback;
    try { return JSON.stringify(obj, null, 2); } catch { return fallback; }
}

function renderAgentPanel(payload, source) {
    if (!agentPanelWidget) return;
    openAgentPanel();

    const status = payload?.status || source || 'agent';
    const mainText = payload?.result || payload?.clarification_question || payload?.detail || 'No detailed output.';

    if (agentPanelSummary) {
        agentPanelSummary.textContent = `${status}: ${String(mainText).slice(0, 500)}`;
    }

    if (agentPanelContract) {
        const contract = payload?.contract || payload?.internal_contract || payload?.plan || null;
        agentPanelContract.textContent = formatJsonCompact(contract);
    }

    if (agentPanelEval) {
        const evalObj = payload?.evaluation || {
            confidence_score: payload?.confidence,
            citations: payload?.citations,
        };
        agentPanelEval.textContent = formatJsonCompact(evalObj);
    }

    if (agentPanelSteps) {
        const steps = payload?.step_results || payload?.plan?.steps || payload?.raw_plan || null;
        agentPanelSteps.textContent = typeof steps === 'string' ? steps : formatJsonCompact(steps);
    }
}

async function runAgentPlanTask() {
    if (isStreaming) return;
    const query = (messageInput.value || '').trim() || window.prompt('Plan task for agent:', 'Create architecture implementation plan');
    if (!query) return;
    ...
    try {
        const res = await fetch(API + '/agent/plan', {
            ...
            body: JSON.stringify({
                user_query: query,
                forced_mode: currentAgentMode,
                session_id: sessionId,
                context: { route_mode: currentMode, source: 'vanilla_frontend' },
            }),
        });
        ...
        renderAgentPanel(data, 'plan_ready');
    } catch (err) {
        renderAgentPanel({ status: 'error', detail: 'Agent plan failed: ' + String(err.message || err) }, 'error');
    }
}
...
function bindEvents() {
    ...
    if (agentExecuteBtn) agentExecuteBtn.addEventListener('click', () => { void runAgentExecuteTask(); });
    ...
    if (btnAgentCasual) btnAgentCasual.addEventListener('click', () => setAgentMode('casual'));
    if (btnAgentStrategic) btnAgentStrategic.addEventListener('click', () => setAgentMode('strategic'));
    ...
    document.querySelectorAll('.welcome-chips .chip').forEach(c => {
    ...
    if (agentPanelToggle) {
        agentPanelToggle.addEventListener('click', openAgentPanel);
    }
    if (agentPanelClose) {
        agentPanelClose.addEventListener('click', closeAgentPanel);
    }
    if (agentPlanBtn) {
        agentPlanBtn.addEventListener('click', () => { void runAgentPlanTask(); });
    }
    if (agentExecBtnPanel) {
        agentExecBtnPanel.addEventListener('click', () => { void runAgentExecuteTask(); });
    }
}
...
function setAgentMode(mode) {
    currentAgentMode = mode === 'casual' ? 'casual' : 'strategic';
    if (btnAgentCasual) btnAgentCasual.classList.toggle('active', currentAgentMode === 'casual');
    if (btnAgentStrategic) btnAgentStrategic.classList.toggle('active', currentAgentMode === 'strategic');
    updateModeMetaLabel();
}

function updateModeMetaLabel() {
    if (!modeLabel) return;
    const routeLabel = currentMode === 'general' ? 'General' : 'Realtime';
    const agentLabel = currentAgentMode === 'casual' ? 'Casual' : 'Strategic';
    modeLabel.textContent = routeLabel + ' | Agent: ' + agentLabel;
}

function applyAgentModeHint(text) {
    const clean = String(text || '').trim();
    if (!clean) return clean;
    if (currentAgentMode === 'casual') return 'Switch to casual mode\n' + clean;
    return 'Switch to strategic mode\n' + clean;
}
 ...
 */
async function runAgentExecuteTask() {
    if (isStreaming) return;
    const instruction = (messageInput.value || '').trim() || window.prompt('Agent instruction:', 'Analyze and execute safely');
    ...
    try {
        const payload = {
            ...
            session_id: sessionId,
            context: {
                route_mode: currentMode,
                source: 'vanilla_frontend',
            },
        };
        ...
        if (res.status === 409) {
            const body = await res.json().catch(() => ({}));
            const ok = window.confirm((body && body.detail) || 'Action requires confirmation. Continue?');
            if (ok) {
        ...
        addMessage('assistant', data.result || data.clarification_question || 'Agent task completed.');
        renderAgentPanel(data, data.status || 'execute_done');
    } catch (err) {
        removeTypingIndicator();
        addMessage('assistant', 'Agent execute failed: ' + String(err.message || err));
    } finally {
        isStreaming = false;
...
async function sendMessage(textOverride) {
    ...
    try {
        ...
        const res = await fetch(`${API}${endpoint}`, {
            ...
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                message: applyAgentModeHint(text),             // User message with agent mode hint
